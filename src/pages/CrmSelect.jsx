...
export default function CrmSelect({ admin, activatingCrmKey = '', error = '', onSelect, onLogout }) {
  return (
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              {CRM_OPTIONS.map((crm) => (
                <button
                  key={crm.key}
                  type="button"
                  disabled={Boolean(activatingCrmKey)}
                  onClick={() => onSelect(crm.key)}
                  className="group rounded-lg border border-white/10 bg-white p-5 text-left text-ink shadow-soft transition hover:-translate-y-0.5 hover:border-teal-300 disabled:cursor-wait disabled:opacity-70"
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="rounded-lg bg-slate-100 p-2 text-slate-700 transition group-hover:bg-teal-50 group-hover:text-teal-700">
                      {activatingCrmKey === crm.key ? '...' : <ArrowRight size={20} />}
                    </span>
                  </div>
                  <p className="mt-4 text-sm leading-6 text-slate-600">
                    {activatingCrmKey === crm.key ? 'Activando operacion para WhatsApp...' : crm.description}
                  </p>
                </button>
              ))}
            </div>
